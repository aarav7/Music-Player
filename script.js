...
for (i = 0; i < songList.length; i++) {
    ...
    songList[i].getElementsByTagName("span")[0].setAttribute("title", songs[i].songName);
...
function displayInfo() {
    for (i = 0; i < playSongs.length; i++) {
        if (parseInt(playSongs[i].id) == songId) {
            ...
            songInfo[0].getElementsByTagName("span")[1].innerText = playSongs[i].previousElementSibling.innerText;
        }
    }
}

function remove() {
    bottom[0].style.transform = "translateY(100%)";
}
